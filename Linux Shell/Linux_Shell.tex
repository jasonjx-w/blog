...
\subparagraph{管道}用$|$隔开，前一文件的结果作为后一命令的输入
\subparagraph{后台运行}命令后接$\&$符号


可以每条命令占一行；也可以将命令后接分号，写在一行。

$\&\&$与符号，表示前一条命令若执行成功，再执行后一条命令。
$||$或符号，表示前一条命令若执行不成功，再执行后一条命令。

数组：数组名=（值1 值2 值3 ...），可以使用*和@号表示数组中所有下标。
\begin{verbatim}
